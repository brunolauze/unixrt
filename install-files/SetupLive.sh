...
MDSIZE="`/bin/du -m /dist/UNIXRT.ufs.uzip | /bin/cut -f 1`"
MDSIZE="`/bin/expr ${MDSIZE} + 300`"
MDCHECK="`/bin/expr ${MDSIZE} + 512`"
if [ $MEM -lt $MDCHECK ]
then
  # We will run the uzip file from DVD/USB directly, boo!
  echo "Mounting UNIXRT.ufs.uzip.."
  # Mount the  "usr" directory
  echo "Mounting /usr"
  MDDEVICE="`/sbin/mdconfig -a -t vnode -o readonly -f /dist/UNIXRT.ufs.uzip`.uzip"
  ...
  /bin/cp /dist/UNIXRT.ufs.uzip /memfs/

  echo "Mounting /usr"
  mkdir /mntuzip >/dev/null 2>/dev/null
  MDDEVICE="`/sbin/mdconfig -a -t vnode -o readonly -f /memfs/UNIXRT.ufs.uzip`.uzip"
  /sbin/mount -r /dev/$MDDEVICE /mntuzip
fi
